...
def run_agentic_workflow(startup_data: dict):
    ...
    research_task = Task(
        description=f"Based on the initial analysis provided in the context, conduct up-to-date market and competitor research for '{startup_data['startup_name']}'. Focus on market size, trends, and key competitors.",
        expected_output="A detailed market research report including current market size, competitor landscape, and recent trends.",
        agent=research_agent_instance.agent
    )

    content_task = Task(
        description=f"Create a full, investor-ready pitch deck for '{startup_data['startup_name']}'. Synthesize the user's initial data and the provided analyses into a cohesive and compelling narrative. Use the 'startup_data' for the company's specific details.",
        expected_output="A complete, well-structured pitch deck in a clean, readable text format. Each slide should have a title and key bullet points.",
...
def display_main_form():
    ...
    st.markdown('<p class="sub-header">Powered by a Multi-Agent CrewAI System</p>', unsafe_allow_html=True)
    st.markdown("""
    <div class="workflow-info">
        <strong>Workflow:</strong> Direct RAG → Research Agent → Content Agent → Final Pitch Deck
    </div>
    """, unsafe_allow_html=True)
...
def display_results():
    st.header("Generated Pitch Deck Content")
    
    if 'crew_result' in st.session_state:
        st.markdown(st.session_state.crew_result)

    if 'source_document' in st.session_state and st.session_state.source_document:
        with st.expander("View Source Context Used by AI"):
            st.text_area(label="Source Context", value=st.session_state.source_document, height=300, disabled=True)
